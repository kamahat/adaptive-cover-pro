...
class AdaptiveDataUpdateCoordinator(DataUpdateCoordinator[AdaptiveCoverData]):
    ...
    def get_blind_data(self, options):
        ...
        # kamahat: use -1.0 (below horizon) when sun.sun unavailable to prevent
        # spurious cover commands at HA startup (0.0 elevation = valid FOV hit)
        if _raw_elev is None:
            self.logger.warning(
                "sun.sun elevation unavailable, using -1.0 to suppress solar commands"
            )
        sol_elev = _raw_elev if _raw_elev is not None else -1.0
