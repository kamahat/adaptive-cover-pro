# v2.23.0-beta.4 — set_position accepts HA target keys on cumulative beta.3 base

beta.4 is a small fix release on top of beta.3: it corrects `adaptive_cover_pro.set_position` to accept standard HA `target:` blocks (entity, device, or area selectors), which previously failed with a schema validation error ([#460], [#461]). All beta.3, beta.2, and beta.1 content is included below for users upgrading from stable.

## Fix in beta.4

### `adaptive_cover_pro.set_position` accepts HA target keys ([#460], [#461])

Calling `adaptive_cover_pro.set_position` with a standard HA `target:` block (entity, device, or area selector) failed with a schema validation error. The service worked only when the caller embedded `entity_id` inside the data dict and avoided the `target:` block.

`SET_POSITION_SCHEMA` was defined as `vol.Schema(..., extra=vol.PREVENT_EXTRA)`, which rejected the `entity_id` / `device_id` / `area_id` keys that HA's service target resolution injects into the call data. The schema now uses `cv.make_entity_service_schema(...)` — HA's canonical helper for service schemas that need to accept target keys. New regression tests in `tests/test_services_set_position.py` (`test_schema_accepts_ha_injected_entity_id`, `test_schema_accepts_ha_injected_device_id`, `test_schema_accepts_ha_injected_area_id`) pin each target-key shape.

---

beta.3 is a small polish release on top of beta.2: it refines the French label for `cloud_suppression` in the decision trace ([#457], [#459]) and bumps an internal release-script warning threshold. All beta.2 and beta.1 content is included below for users upgrading from stable.
...
beta.2 adds three new features — `adaptive_cover_pro.stop`, the `manual_ignore_external` option, and translatable `climate_status` state slugs — and fixes two related `sunset_position` bugs. This is a cumulative beta: users upgrading directly from a v2.22 stable will see all changes from beta.1 as well. Feedback welcome before this promotes to stable, particularly around `manual_ignore_external` in mixed-automation environments.

## 🎯 Highlights

- **`set_position` target keys** — `adaptive_cover_pro.set_position` now accepts HA service `target:` blocks (entity, device, or area selectors); the schema previously rejected those as extra keys.
- **`adaptive_cover_pro.stop`** — a new service that routes `cover.stop_cover` through the ACP command stack, stamps an ACP-originated HA context, and engages manual override. Required for the Lovelace card's stop button when `manual_ignore_external` is on; direct `cover.stop_cover` calls bypass the context stamp and get treated as external noise.
- **`manual_ignore_external`** — a new Manual Override option (default off) that limits override engagement to commands that pass through ACP itself: the proxy cover entity and the `adaptive_cover_pro.set_position` / `adaptive_cover_pro.stop` services. RF remotes, other automations, and the default HA cover card no longer pause automatic control when this option is on.
- **Translatable `climate_status`** — `sensor.<cover>_climate_status` now emits `summer_mode`, `winter_mode`, and `intermediate` slugs instead of raw English strings, and HA renders the per-language label from the translation file. This is a breaking change for automations that compared against the old English state values.
- **Sunset position fixes** — two cooperating bugs fixed: the cover sat at `sunset_position` during morning operational hours when sunrise landed after `start_time`, and clearing `sunset_position` back to "no value" in the UI was silently ignored (the stored 0 survived the clear).
- **Position forecast sensor** (from beta.1) — `sensor.<cover>_position_forecast` shows the next forecast event as its state and carries a 12-hour position strip; five cooperating bugs that kept it frozen at boot-time are fixed.
...
_(Beta.1 feature — included here for users upgrading from stable.)_
...
_(Beta.4 fix — described above; included here for completeness.)_

Schema switched from `vol.Schema(..., extra=vol.PREVENT_EXTRA)` to `cv.make_entity_service_schema(...)` so HA-injected `entity_id` / `device_id` / `area_id` keys validate.
...
_(Beta.1 fixes — included here for users upgrading from stable.)_
...
- **O(1) forecast index lookup ([#442], [#447]):** `_nearest_index` in `forecast.py` replaces a linear scan with a direct arithmetic expression keyed on `SUN_DATA_STEP_SECONDS = 300` in `const.py`. _(Beta.1)_

- **Shared SunData day cache ([#441], [#445]):** All `SunData` instances at the same location share one `pd.date_range` + astral fill per day via a module-level `_DAY_CACHE` dict in `sun.py`. _(Beta.1)_

- **Recorder exclusion for heavy sensor attributes ([#446]):** `actual_positions`, `actual_distances`, `position_explanation`, `manual_covers`, `trace`, `custom_position_slots`, `enabled_handlers`, `forecast`, `events`, and `per_entity` are excluded from HA recorder writes. Live state, templates, and the Lovelace card are unaffected. _(Beta.1)_

- **Enums consolidated into `const.py`:** `enums.py` deleted; `CoverType`, `TiltMode`, `TemperatureSource`, `PresenceDomain`, `ClimateStrategy`, `ControlMethod`, and forecast constants moved into `const.py`. No behavior change. _(Beta.1)_
...
- 3 new tests in `tests/test_services_set_position.py` pinning HA target-key acceptance: `test_schema_accepts_ha_injected_entity_id`, `test_schema_accepts_ha_injected_device_id`, `test_schema_accepts_ha_injected_area_id` (#460).

New in beta.3:

- `test_fr_cloud_suppression_decision_trace_state` regression guard in `tests/test_translations.py`: asserts the corrected French label for `cloud_suppression` in the `decision_trace` state block ([#457]).

New in beta.2:

- `tests/test_manual_ignore_external.py`: covers both option states across the user-context fast-path, numeric-diff path, stop_cover path, and proxy/service engagement path.
- 12 new tests for the `adaptive_cover_pro.stop` service path.
- 5 new tests for the morning `sunset_position` fallback fix (#438).
- Tests for the cannot-clear-stored-0 fix (#439): `_POSITION_OPTIONAL_KEYS` coverage across both config and options flow handlers.

## Compatibility

- Home Assistant 2026.3.0+
- Python 3.11+
- **Breaking change:** `sensor.<cover>_climate_status` state values changed from English strings to slugs (`summer_mode`, `winter_mode`, `intermediate`). Update any automation or template that compared against the old values.
